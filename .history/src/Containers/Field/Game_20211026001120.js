...
const Game = () => {
  ...
  useEffect(() => {
    ws.connect(params.id, isSecondary, (event) => {
      ...
      switch (msg.type) {
        ...
        case "READYTOPLAY":
          //console.log("RECEIVED_MSG", msg);
          readeToplay(msg.payload);
          console.log("все таки что то приходит");
          break;
        case "CONNECTED":
          console.log("---->>>CONNECTED_MSG", msg.myIdToEnemyId);
          onConnection(msg.myIdToEnemyId);
          break;
        default:
          break;
      }
  ...
  const onConnection = (myIdToEnemyId) => {
    setEnemyId(myIdToEnemyId);
    localStorage.setItem("EnemyIDII", myIdToEnemyId);
  };

  const readeToplay = (myIdToEnemyId) => {
    setEnemyId(myIdToEnemyId);
    localStorage.setItem("EnemyIDII", myIdToEnemyId);
  };
