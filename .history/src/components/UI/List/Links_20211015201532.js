...
export default class Links extends Component {
  ...
  joinGame = async (event) => {
    ...
    let string = {
      cliced: Number(event.target.number) + 1,
      id: event.target.id,
    };
          ...
          <ul className={classes.li} key={Math.random()}>
            <li ref={this.linkRef}>
              <a
                ...
                id={link._id}
                number={link.cliced} // вот это место очень непонятное, я просто не знал какой property использовать у тега <a> для передачи NUMBER и использовал elementTiming
              >
